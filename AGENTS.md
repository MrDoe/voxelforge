...
> **Scope (current session):** We focus **only on the voxel (SVO ray-march) render output** — `SvoPass` / `shaders/svo_raymarch.comp` / `LayeredWorld` SVO synthesis — until the user explicitly states otherwise. Splats, billboards, probes, and the dense `RaymarchPass` (`raymarch.comp`) are out of scope for now. Keep `raymarch.comp` in parity with `svo_raymarch.comp` only where required by `--compare`.
...
- The app loads the layers dynamically via `src/voxel/layered_world.cpp` (`LayeredWorld`): reads `world.json`, merges enabled layers with priority, then **synthesizes the chunked-SVO `GpuWorld` directly from the voxels** (object shells are flood-filled into solid volumes via per-component 3D air flood-fill, terrain interior from `heightAt`, air gets a BFS distance for sphere-tracing). Every renderer (SVO ray-march, splats, probes) consumes this single `GpuWorld`, so a layer edit / enable-toggle / MCP append is reflected everywhere at once. No recompile needed. If `world.json`/a layer file changes on disk, `reloadIfChanged()` rebuilds within ~0.5 s (the app polls mtimes each frame).
...
- `src/voxel/layered_world.{hpp,cpp}` — the single runtime world source: `LayeredWorld` merges enabled `world.json` layers (priority dedupe, first layer wins a cell) and synthesizes the chunked-SVO `GpuWorld` from records. Object shells are flood-filled to **solid** volumes (per-connected-component 3D air flood-fill: label object shells into components, flood air inward from each component's bbox, unreached cells = interior) so trunks/foliage/houses/rocks/AI edits read solid at every distance; air cells get a small-window BFS distance to the nearest solid + inherited seed appearance; air below `WATER_LEVEL` is water volume. `reloadIfChanged()` rebuilds on disk change.
