...
bool VolumetricFogPass::init(const Context& ctx)
{
    ...
    auto spirv = loadSpirv(std::string(VOXELFORGE_SHADER_DIR) + "/volumetric_fog.comp.spv");
...
void VolumetricFogPass::updateDescriptors(VkImageView hdrView, VkImageView gposView, VkImageView outView)
{
...
void VolumetricFogPass::record(VkCommandBuffer cmd, uint32_t width, uint32_t height, const RaymarchPush& push) const
{
