...
bool MotionBlurPass::init(const Context& ctx)
{
    ...
    VkPushConstantRange pc{ VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(MotionBlurPass::PC) }; // must match the shader PC block (128 B)
    ...
    auto spirv = loadSpirv(std::string(VOXELFORGE_SHADER_DIR) + "/motion_blur.comp.spv");
...
void MotionBlurPass::updateDescriptors(VkImageView currentView, VkImageView gposView, VkImageView outView)
{
    if (!m_set || !m_ctx) return;
    VkDescriptorImageInfo i0{ VK_NULL_HANDLE, currentView, VK_IMAGE_LAYOUT_GENERAL };
...
void MotionBlurPass::record(VkCommandBuffer cmd, uint32_t width, uint32_t height,
                            const RaymarchPush& push, const TaaPrevCam& prev) const
{
    ...
    PC pc{};
    static_assert(sizeof(PC) == sizeof(RaymarchPush) + 5 * 16);
    pc.base = push;
    pc.prevPos[0] = prev.pos.x; pc.prevPos[1] = prev.pos.y; pc.prevPos[2] = prev.pos.z;
    pc.prevRight[0] = prev.right.x; pc.prevRight[1] = prev.right.y; pc.prevRight[2] = prev.right.z;
    pc.prevUp[0] = prev.up.x; pc.prevUp[1] = prev.up.y; pc.prevUp[2] = prev.up.z;
    pc.prevFwd[0] = prev.fwd.x; pc.prevFwd[1] = prev.fwd.y; pc.prevFwd[2] = prev.fwd.z;
    pc.prevParams[0] = prev.tanHalfFov; pc.prevParams[1] = prev.aspect;
