...
bool DepthOfFieldPass::init(const Context& ctx)
{
    ...
    VkPushConstantRange pc{ VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(DepthOfFieldPass::PC) }; // must match the shader PC block (128 B)
    ...
    auto spirv = loadSpirv(std::string(VOXELFORGE_SHADER_DIR) + "/depth_of_field.comp.spv");
...
void DepthOfFieldPass::updateDescriptors(VkImageView sceneView, VkImageView gposView, VkImageView outView)
{
    if (!m_set || !m_ctx) return;
    VkDescriptorImageInfo i0{ VK_NULL_HANDLE, sceneView, VK_IMAGE_LAYOUT_GENERAL };
...
void DepthOfFieldPass::record(VkCommandBuffer cmd, uint32_t width, uint32_t height,
                              const RaymarchPush& push, float focusDist, float focalLength) const
{
    ...
    PC pc{};
    static_assert(sizeof(PC) == sizeof(RaymarchPush) + 16);
    pc.base = push;
    pc.focus[0] = focusDist;
    pc.focus[1] = focalLength;
