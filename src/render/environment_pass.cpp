...
bool EnvironmentPass::init(const Context& ctx)
{
    ...
    auto spirv = loadSpirv(std::string(VOXELFORGE_SHADER_DIR) + "/env_map.comp.spv");
    ...
    VkDescriptorPoolSize sizes[1] = { { VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 2 } };
...
void EnvironmentPass::updateDescriptors(VkImageView srcView, VkImageView envView)
{
    if (!m_set || !m_ctx) return;
    VkDescriptorImageInfo i0{ VK_NULL_HANDLE, srcView, VK_IMAGE_LAYOUT_GENERAL };
    VkDescriptorImageInfo i1{ VK_NULL_HANDLE, envView, VK_IMAGE_LAYOUT_GENERAL };
...
void EnvironmentPass::record(VkCommandBuffer cmd, uint32_t width, uint32_t height, int face, int mip, int res, const RaymarchPush& push) const
{
    RaymarchPush pushData = push;
    pushData.misc.x = float(face);
    pushData.misc.y = float(mip);
    pushData.misc.z = float(res);
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_pipeline);
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, m_layout, 0, 1, &m_set, 0, nullptr);
    vkCmdPushConstants(cmd, m_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(RaymarchPush), &pushData);
