...
namespace aidetail {
struct AiCells {
    std::unordered_map<uint32_t, uint8_t> mat;
    int lo[3] = { 0, 0, 0 };
    int hi[3] = { -1, -1, -1 }; // empty when hi < lo
};
inline AiCells& aiCells()
{
    static AiCells m;
    return m;
}
inline uint32_t cellKey(int x, int y, int z)
{
    return (uint32_t(x) << 20) | (uint32_t(y) << 10) | uint32_t(z);
}
} // namespace aidetail

inline void aiEditsClear()
{
    auto& m = aidetail::aiCells();
    m.mat.clear();
    m.hi[0] = m.hi[1] = m.hi[2] = -1;
}
inline void aiEditsRegister(int x, int y, int z, uint8_t mat)
{
    auto& m = aidetail::aiCells();
    m.mat[aidetail::cellKey(x, y, z)] = mat;
    int v[3] = { x, y, z };
    for (int a = 0; a < 3; ++a) {
        if (m.hi[a] < m.lo[a]) {
            m.lo[a] = v[a];
            ...
            m.lo[a] = std::min(m.lo[a], v[a]);
            m.hi[a] = std::max(m.hi[a], v[a]);
        }
    }
}
inline size_t aiEditsCount()
{
    return aidetail::aiCells().mat.size();
}

// signed distance to registered AI-edit cubes. Outside their AABB: exact
...
inline ObjHit aiEditsAt(glm::vec3 p, uint64_t mask = OB_ALL)
{
    ...
    glm::vec3 bmin(-0.5f * WORLD + float(m.lo[0]) * VOXEL - VOXEL * 0.5f,
                   -0.5f * WORLD + float(m.lo[1]) * VOXEL - VOXEL * 0.5f,
                   -0.5f * WORLD + float(m.lo[2]) * VOXEL - VOXEL * 0.5f);
    glm::vec3 bmax(-0.5f * WORLD + float(m.hi[0]) * VOXEL + VOXEL * 0.5f,
                   -0.5f * WORLD + float(m.hi[1]) * VOXEL + VOXEL * 0.5f,
                   -0.5f * WORLD + float(m.hi[2]) * VOXEL + VOXEL * 0.5f);
    glm::vec3 q = glm::max(bmin - p, p - bmax);
    if (q.x > 0.0f || q.y > 0.0f || q.z > 0.0f)
        return { glm::max(glm::length(glm::max(q, glm::vec3(0.0f))) - 0.087f, 0.0f),
                 4u };

    int cc[3] = { int(std::floor((p.x + 0.5f * WORLD) / VOXEL)),
    ...
    for (int ox = -1; ox <= 1; ++ox)
        for (int oy = -1; oy <= 1; ++oy)
            for (int oz = -1; oz <= 1; ++oz) {
                auto it =
                    m.mat.find(aidetail::cellKey(cc[0] + ox, cc[1] + oy, cc[2] + oz));
                if (it == m.mat.end())
                    continue;
                glm::vec3 center(-0.5f * WORLD + float(cc[0] + ox) * VOXEL,
                                 -0.5f * WORLD + float(cc[1] + oy) * VOXEL,
                                 -0.5f * WORLD + float(cc[2] + oz) * VOXEL);
                float d = sdBoxF(p, center, glm::vec3(VOXEL * 0.5f));
                if (d < best.d) {
                    best.d = d;
                    best.mat = it->second;
